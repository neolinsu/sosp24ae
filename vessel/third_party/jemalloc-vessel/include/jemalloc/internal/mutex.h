...
#include "jemalloc/internal/lock.h"
#endif
...
#elif (!defined(VESSEL_STARTUP))
#    define MALLOC_MUTEX_LOCK(m)    spin_lock(&(m)->lock)
#    define MALLOC_MUTEX_UNLOCK(m)  spin_unlock(&(m)->lock)
#    define MALLOC_MUTEX_TRYLOCK(m) (spin_try_lock(&(m)->lock) == 0)
#else
...
#  if defined(JEMALLOC_DEBUG)
#    define MALLOC_MUTEX_INITIALIZER					\
     {{{LOCK_PROF_DATA_INITIALIZER, SPINLOCK_INITIALIZER, ATOMIC_INIT(false)}}, \
           WITNESS_INITIALIZER("mutex", WITNESS_RANK_OMIT), 0}
#  else
#    define MALLOC_MUTEX_INITIALIZER                          \
     {{{LOCK_PROF_DATA_INITIALIZER, SPINLOCK_INITIALIZER, ATOMIC_INIT(false)}},	\
