...
static void* mmap_top=0;
static void* mmap_limit=0;
static volatile int mmap_lock=0;
static void fetch_mmap_area(void **addr, void **limit);
#ifndef _WIN32
...
os_pages_map(void *addr, size_t size, size_t alignment, bool *commit) {
	 ...
	 */
	{
		...
		while (!(__sync_bool_compare_and_swap(&mmap_lock, 0, 1)));
		//ret = mmap(addr, size, prot, mmap_flags, PAGES_FD_TAG, 0);
		ret =  (void*)((size_t)mmap_top& (~(alignment-1)));
		if(ret!= mmap_top) {
			ret = ((char*)ret+alignment);
		}
		if((char*)ret+size>= (char*)mmap_limit) {
			fetch_mmap_area(&mmap_top, &mmap_limit);
			ret =  (void*)((size_t)mmap_top& (~(alignment-1)));
			if(ret!= mmap_top) {
				ret = ((char*)ret+alignment);
			}
			if((char*)ret+size > (char*)mmap_limit) {
				malloc_printf("<jemalloc>: Cannot allocate memory in os_pages_map it is the last level failure!!!\n");
				mmap_lock=0;
				return NULL;
			}
		} 
		mmap_top = (char*)ret+size;

		// malloc_printf("mmap: the addr is %p, the size is %ld, the return is %p\n", addr, size,ret);
		__sync_synchronize();
		mmap_lock=0;
		memset(ret,0,size);
	}
...
os_pages_unmap(void *addr, size_t size) {
	//malloc_printf("unmap: the addr is %p, the size is %ld\n", addr, size);
	assert(ALIGNMENT_ADDR2BASE(addr, os_page) == addr);
	assert(ALIGNMENT_CEILING(size, os_page) == size);
	while (!__sync_bool_compare_and_swap(&mmap_lock, 0, 1));
	memset(addr,0,size);

	if ((char*)addr == (char*)mmap_top-size) {
		mmap_top = addr;
	}
	mmap_lock=0;
	return ;
...
pages_map(void *addr, size_t size, size_t alignment, bool *commit) {
	...
	void *ret = os_pages_map(addr, size, alignment, commit);
...
static const vessel_apis_t *vapis= (vessel_apis_t *)0x8d232008;
...
static void fetch_mmap_area(void **addr, void **limit) {
	...
	// malloc_printf("trying to fetch mmap area from vessel success\n");
    erim_switch_to_untrusted(0);
	*addr = ctl.allocated_o;
#endif
#ifdef VESSEL_STARTUP
	size_t chunk_num = 1; // 1G
	*addr = startup_vessel_alloc_task_chunk(chunk_num);
#endif
	assert(addr != NULL);
	*limit = *addr + ((size_t) chunk_num *1024*1024*1024);
	// malloc_printf("mmap area: %p - %p\n", *addr, *limit);
	return;
}
