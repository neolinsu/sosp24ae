...
class Bash:
    @classmethod
    def run_cmd_in_bash(cls, bash, cmd:str, expect_str:str, timeout:int=10, cmd_timeout:int=10)->bool:
        bash.sendline(cmd)
        # Take in the cmd itself.
        index = bash.expect([pexpect.TIMEOUT, os.linesep], timeout=cmd_timeout)
        if (index == 0):
            print(f"Fail to run bash with '{cmd}', expect '{expect_str}'.")
            return False
        if expect_str:
    ...
    def __init__(self, set_up_bash:str, set_up_expect_str:str, password:str, remotehost:str=None, need_su:bool=True):
        ...
        if need_su:
            if password is None:
                raise RuntimeError("password is needed")
        ...
        if not set_up_bash is None:
            ret = self.run_cmd_in_bash(process, set_up_bash, set_up_expect_str, timeout=3)
            if not ret:
                self.process.interact()

    def run_cmd(self, cmd:str, expect_str:str, timeout:int=20):
        ...
        if expect_str:
            ...
            if(ret!=1):
                self.process.interact()
                raise RuntimeError(f"Fail to run bash with '{cmd}', expect '{expect_str}'.")
        return
    ...
    def is_alive(self):
        return (hasattr(self, "process")) and not self.process is None and self.process.isalive()
