...
#     outputfile1 = f"output_gsw/memb_duration_{i}.txt"
#     outputfile2 = f"output_gsw/memb_duration_bw_{i}.txt"
...
#     x2.kill()
def get_bandwidth(i):
    f = open(f"output_gsw/memb_duration_{i}.txt")
    idx = 0
    ans = [0] * 2
    for line in f:
...
for i in range (10,110, 10):
    print(f"Targe bandwidth percent:{i}%, Tested bandwidth:{get_bandwidth(i)/max_bandwidth}%")