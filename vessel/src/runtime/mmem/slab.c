		    ...
		    int offset, int flags, int nr_elems)
{
	...
	bitmap_for_each_set(cpu_info_tbl.numa_mask, NNUMA, i) {
		log_info("__slab_early_create on %i", i);
...
static int __slab_early_migrate(struct slab *s)
{
	...
	bitmap_for_each_set(cpu_info_tbl.numa_mask, NNUMA, i) {
		log_info("__slab_early_migrate on %i", i);
		n = (struct slab_node *)slab_alloc_on_node(&node_slab, i);
		if (!n)
			goto fail;
...
static void slab_item_check(struct slab_node *n, void *item)
{
	...
	if (unlikely(!is_thread_init_done()))
		return;

	/* NUMA node checks */
	assert(n->numa_node == my_thread_numa_node());
	assert(addr_to_numa_node(item) == my_thread_numa_node());
...
static int slab_tcache_alloc(struct tcache *tc, int nr, void **items)
{
	...
	struct slab_node *n = s->nodes[my_thread_numa_node()];
	int i;
...
void slab_print_usage(void)
{
	...
	list_for_each(&slab_list, s, link) {
		...
		bitmap_for_each_set(cpu_info_tbl.numa_mask, NNUMA, i) {
			log_info("slab_print_usage on %d", i);
...
int slab_init(void)
{
	...
	if (ret)
		return ret;
	// log_info("Slab init: 3");
	ret = __slab_early_migrate(&smpage_slab);
	if (ret)
		return ret;
	// log_info("Slab init: 4");
	/*
