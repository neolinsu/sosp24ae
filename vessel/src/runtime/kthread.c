...
static struct kthread *allock(void)
{
	...
	k = mini_malloc_C(align_up(sizeof(*k), CACHE_LINE_SIZE));
	if (!k)
		return NULL;
...
{
	struct kthread *k = myk();
	// log_info("k->tid: %d, fs:%p", k->tid, get_cur_fs());

	vessel_cluster_mwait();
	k->curr_cpu = get_cur_cpuid();
