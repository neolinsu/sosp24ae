...
static int parse_runtime_kthreads(const char *name, const char *val)
{
	...
	if (tmp < 1 || tmp > NCPU - 1) {
		...
		log_err("must be > 0 and < %d (number of CPUs)", NCPU);
		return -EINVAL;
	}
...
static int parse_runtime_guaranteed_kthreads(const char *name, const char *val)
{
	...
	if (tmp > NCPU - 1) {
		...
		log_err("must be < %d (number of CPUs)", NCPU);
		return -EINVAL;
	}
...
static int parse_preferred_socket(const char *name, const char *val)
{
	...
	if (tmp < 0 || tmp > NNUMA - 1) {
		log_err("invalid preferred socket requested, '%ld'", tmp);
		log_err("must be >=0 and < %d (number of NUMA nodes)", NNUMA);
		return -EINVAL;
	}
...
static int parse_enable_cxltp(const char *name, const char *val)
{
#ifdef VESSEL_CXLTP
	cfg_cxltp_enabled = true;
	return 0;
#else
	log_err("cfg: cannot enable cfg_cxltp, "
		"please recompile with cfg_cxltp support");
	return -EINVAL;
#endif
}
...
static int parse_numa_nodeid(const char *name, const char *val)
{
	int ret;
	sscanf(val, "%d", &ret);
	numa_nodeid = ret;
...
static int parse_force_init_cxltp_mem(const char *name, const char *val)
{
#ifdef VESSEL_CXLTP
	force_init_cxltp_mem = true;
#endif
	return 0;
}

static int parse_app_id(const char *name, const char *val)
{
	strncpy(app_id, val, sizeof(app_id) - 1);
...
static const struct cfg_handler cfg_handlers[] = {
	...
	{ "enable_cxltp", parse_enable_cxltp, false },
	{ "enable_directpath", parse_enable_directpath, false },
	{ "enable_gc", parse_enable_gc, false },
	{ "numa_nodeid", parse_numa_nodeid, true },
	{ "force_init_cxltp_mem", parse_force_init_cxltp_mem, false },
	{ "app_id", parse_app_id, true },
};
