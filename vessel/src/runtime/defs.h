...
static inline pid_t thread_gettid(void)
{
#ifndef SYS_gettid
...
extern void __jmp_thread(vcontext_t *tf) __noreturn;
extern void __jmp_thread_hard_ret(vcontext_t *tf) __noreturn;
extern void __jmp_thread_direct(vcontext_t *oldtf,
				vcontext_t *newtf,
				unsigned int *thread_running);
extern void __jmp_thread_direct_hard_ret(vcontext_t *oldtf,
				vcontext_t *newtf,
				unsigned int *thread_running);
extern void __jmp_runtime(vcontext_t *tf, runtime_fn_t fn,
			  void *stack);
...
BUILD_ASSERT(sizeof(tqueue_t) == CACHE_LINE_SIZE);
...
BUILD_ASSERT(sizeof(struct tqueue_elem) == CACHE_LINE_SIZE);
...
struct kthread {
	...
	thread_t		*cxltp_softirq;
	thread_t		*storage_softirq;
	bool			iokernel_busy;
	bool			directpath_busy;
	bool			directpath_stealable;
	bool			timer_busy;
	bool			cxltp_busy;
	bool			storage_busy;
	unsigned int		pad2[0];
	...
	/* 11th cache-line, cxltp */
	struct cxltp_rx cxltp_rx;
...
extern bool softirq_steal(struct kthread *k);
...
#ifdef VESSEL_CXLTP
extern bool cfg_cxltp_enabled;
extern char cxltp_arg[128];
extern bool force_init_cxltp_mem;
#endif
...
extern int thread_init_thread(void);
...
extern int cxltp_init_thread(void);
...
extern int minialloc_init(void);
extern int stack_init(void);
extern int cxltp_init(void);
...
extern void thread_cede_hard(void);
