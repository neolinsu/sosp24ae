...
void tcp_rx_conn(struct trans_entry *e, struct mbuf *m)
{
	...
	spin_lock_np(&c->lock);
	// log_info_ratelimited("packet ack %u win %u seq %u\n,pcb una %u nxt%u wind %u",
	// 	ack,win,seq,c->pcb.snd_una,c->pcb.snd_nxt,c->pcb.snd_wnd);
	      ...
	      uint32_t win, const unsigned char *optp, int optlen)
{
	...
	uint32_t seq, len;
	bool do_ack = false, do_drop = true, fin = false, snd_was_full, not_acceptable=false;
...
do_ack:
	...
	snd_was_full = tcp_is_snd_full(c);
	// log_info_ratelimited("c->pcb.snd_una: %u, ack: %u, snd_nxt: %u, win: %d"
	// ", laddr: %x, lport: %d, raddr: %x, rport: %d", c->pcb.snd_una, ack, snd_nxt, win,
	// c->e.laddr.ip, c->e.laddr.port, c->e.raddr.ip, c->e.raddr.port);
