...
static void tcp_retransmit_nowait(tcpconn_t *c);
/* check for timeouts in a TCP connection */
void do_tcp_handle_timeouts(tcpconn_t *c, uint64_t now)
{
	bool do_ack = false, do_probe = false, do_retransmit = false;
...
void tcp_conn_ack(tcpconn_t *c, struct list_head *freeq)
{
	...
	while (true) {
		...
		if (wraps_gt(m->seg_end, c->pcb.snd_una))
			break;
		if (!m->release) {
			log_err("tcp_conn_ack -> !m->release");
		}
...
static ssize_t tcp_read_nowait(tcpconn_t *c, size_t len,
			     struct list_head *q, struct mbuf **mout)
{
	struct mbuf *m;
	size_t readlen = 0;
	...
	*mout = NULL;
	...
	/* block until there is an actionable event */
	if (c->rx_exclusive || list_empty(&c->rxq)) {
		spin_unlock_np(&c->lock);
		return 0;
	}
	/* is the socket closed? */
...
ssize_t tcp_try_read(tcpconn_t *c, void *buf, size_t len)
{
#ifdef DIRECTPATH
	char *pos = buf;
	struct list_head q;
...
static int tcp_write_wait(tcpconn_t *c, size_t *winlen)
{
	...
	while (!c->tx_closed &&
		...
		c->why_i_tx_wait = 0;
		c->why_i_tx_wait |= c->pcb.state < TCP_STATE_ESTABLISHED ? TXWAIT_NOT_ESTAB : 0;
		c->why_i_tx_wait |= c->tx_exclusive ? TXWAIT_EXCLUSIVE : 0;
		c->why_i_tx_wait |= tcp_is_snd_full(c) ? TXWAIT_TX_FULL : 0;
		waitq_wait(&c->tx_wq, &c->lock);
	}
	c->zero_wnd = false;
	/* is the socket closed? */
...
static int tcp_write_nowait(tcpconn_t *c, size_t *winlen)
{
	spin_lock_np(&c->lock);
...
static void tcp_write_finish_nowake(tcpconn_t *c)
{
	struct list_head q;
	struct list_head waiters;
	struct mbuf *retransmit = NULL;
...
ssize_t tcp_try_write(tcpconn_t *c, const void *buf, size_t len)
{
	size_t winlen=0;
	ssize_t ret;

	/* block until the data can be sent */
	ret = tcp_write_nowait(c, &winlen);
	if (ret) {
		if (ret==-EBUSY) {
			return 0;
		}
...
static void tcp_retransmit_nowait(tcpconn_t *c)
{
	...
	if (c->tx_exclusive && c->pcb.state != TCP_STATE_CLOSED) {
		spin_unlock_np(&c->lock);
		return;
	}
