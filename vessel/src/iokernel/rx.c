		...
		uint16_t data_room_size, int socket_id)
{
	...
	if (mp == NULL) {
        log_err("rte_mempool_create_empty fail");
		goto fail;
    }
	...
	log_info("dp.ingress_mbuf_region.base: %p", dp.ingress_mbuf_region.base);
...
#pragma GCC diagnostic pop
	...
	if (!heap_area) {
        log_err("rte_malloc_socket fail");
		goto fail_unmap_memory;
    }
...
fail_free_mempool:
	rte_mempool_free(mp);
fail:
	log_err("rx: couldn't create pktmbuf pool %s for %s", name, rte_strerror(rte_errno));
	return NULL;
}
