...
int sched_idle_on_core(uint32_t mwait_hint, unsigned int core)
{
	...
	// printf("sched_idle_on_core: core %d\n", core);
				...
				bool *hw_busy)
{
	...
	*timer_tsc = calc_delay_tsc(tmp);
	...
	if (sched_measure_hardware_delay(th, &th->directpath_hwq, true)) {
		// log_info_ratelimited("directpath timeout for core: %d", th->core);
		busy = true;
		*hw_busy = true;
	}
	...
	if (sched_measure_hardware_delay(th, &th->storage_hwq, true)) {
		// log_info_ratelimited("storage timeout");
		busy = true;
	}
	*storage_tsc = calc_delay_tsc(th->storage_hwq.busy_since);

	if (th->cxltp_waiters) {
		*cxltp_alive_waiters = watch_cxltp_tqueue(th->cxltp_waiters, 10);
		if (*cxltp_alive_waiters>0) busy = true;
		log_info_once("cxltp_waiters: %p", th->cxltp_waiters);
	}
...
static void sched_report_metrics(struct proc *p, uint64_t delay)
{
	...
	ACCESS_ONCE(info->avg_delay_tsc) = delay;
}
...
static void sched_measure_delay(struct proc *p)
{
	...
	for (i = 0; i < p->thread_count; i++) {
		uint64_t delay=0, rxq_tsc=0, uthread_tsc=0, storage_tsc=0, timer_tsc=0,
					cxltp_alive_waiters = 0;
		bool hw_busy = false;
		struct thread * cur_thread = &p->threads[i];
		...
		*(p->threads[i].qdelay) = delay;
		if (cur_thread->active && delay < min_delay) {
			min_delay = delay;
			min_id = i;
		}
		total_delay += delay;
		//sched_ops->notify_kthread_congested(p, i, delay, hw_busy);
	}
	...
	if (p->active_thread_count > 0) {
		total_delay = (total_delay/1)/(p->active_thread_count);
	} else {
		total_delay /= 1;
	}
	sched_report_metrics(p, total_delay);

	/* notify the scheduler policy of the current delay */
	sched_ops->notify_congested(p, busy, hdelay, parked_thread_busy);

	struct congestion_info *info = p->congestion_info;
	//int rest_count = p->thread_count - p->active_thread_count;
	if (//(rest_count <= 5) &&
		 p->sched_cfg.priority == SCHED_PRIO_LC)
	{
		ACCESS_ONCE(info->kthread_yield_requested) = min_id;	
	}
	else
...
//unsigned long sched_poll_times[10];
//static unsigned long prev=0;

//static void record(int type) {
//	long now = rdtsc();
//	sched_poll_times[type] = now-prev;
//	prev=now;
//}
...
int sched_init(void)
{
	...
	for (i = 0; i < cpu_count; i++) {
		if (iokernel_cpu_info_tbl[i].package == -1 && sched_ops != &numa_ops)
			continue;

		if (allowed_cores_supplied &&
		    !bitmap_test(input_allowed_cores, i))
			continue;
		log_debug("sched_allowed_core: %d", i);
