...
static inline unsigned int atomic_u32_read(const atomic_u32_t *a)
{
	return *((volatile unsigned int *)&a->cnt);
}

static inline void atomic_u32_write(atomic_u32_t *a, unsigned int val)
{
	a->cnt = val;
}

static inline unsigned int atomic_u32_fetch_and_add(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_add(&a->cnt, val);
}

static inline unsigned int atomic_u32_fetch_and_sub(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_sub(&a->cnt, val);
}

static inline long atomic_u32_fetch_and_or(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_or(&a->cnt, val);
}

static inline long atomic_u32_fetch_and_xor(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_xor(&a->cnt, val);
}

static inline long atomic_u32_fetch_and_and(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_and(&a->cnt, val);
}

static inline long atomic_u32_fetch_and_nand(atomic_u32_t *a, unsigned int val)
{
	return __sync_fetch_and_nand(&a->cnt, val);
}

static inline unsigned int atomic_u32_add_and_fetch(atomic_u32_t *a, unsigned int val)
{
	return __sync_add_and_fetch(&a->cnt, val);
}

static inline unsigned int atomic_u32_sub_and_fetch(atomic_u32_t *a, unsigned int val)
{
	return __sync_sub_and_fetch(&a->cnt, val);
}

static inline void atomic_u32_inc(atomic_u32_t *a)
{
	atomic_u32_fetch_and_add(a, 1);
}

static inline void atomic_u32_dec(atomic_u32_t *a)
{
	atomic_u32_sub_and_fetch(a, 1);
}

static inline bool atomic_u32_dec_and_test(atomic_u32_t *a)
{
	return (atomic_u32_sub_and_fetch(a, 1) == 0);
}

static inline bool atomic_u32_cmpxchg(atomic_u32_t *a, unsigned int oldv, unsigned int newv)
{
	return __sync_bool_compare_and_swap(&a->cnt, oldv, newv);
}

static inline unsigned int atomic_u32_cmpxchg_val(atomic_u32_t *a, unsigned int oldv, unsigned int newv)
{
	return __sync_val_compare_and_swap(&a->cnt, oldv, newv);
}